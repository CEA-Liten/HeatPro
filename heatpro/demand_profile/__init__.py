...
def day_length_proportionnal_weight(dates: pd.DatetimeIndex) -> pd.Series:
    """Create a Series attributing a weight to each datetime of the index
    ...
    Returns:
        pd.Series: Series with correct format to be used as weight
    """
    return pd.Series(1 / (dates.daysinmonth), index=dates, name=WEIGHT_NAME_REQUIRED)


def apply_hourly_pattern(
    hourly_index: pd.DatetimeIndex, hourly_mapping: dict[int, float]
) -> pd.DataFrame:
...
def apply_weekly_hourly_pattern(
    hourly_index: pd.DatetimeIndex, hourly_mapping: Callable[[int, int], float]
) -> pd.Series:
    """Provide a Series with correct weight format to disaggregate daily load into hourly load with a hourlt weekly pattern.
    ...
    Returns:
        pd.Series: Series weight correct format
    """
    return (
        pd.Series(hourly_index, index=hourly_index)
        .apply(lambda x: hourly_mapping(x.dayofweek, x.hour))
