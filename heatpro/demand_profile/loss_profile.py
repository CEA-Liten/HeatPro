...
def Y_to_H_thermal_loss_profile(temperatures: pd.DataFrame) -> pd.DataFrame:
    ...
    **Overview**
    ...
    temperature_delta = pd.DataFrame(
        (temperatures[SUPPLY_TEMPERATURE_NAME] + temperatures[RETURN_TEMPERATURE_NAME]) / 2
        - temperatures[SOIL_TEMPERATURE_NAME],
    ...
    weights[WEIGHT_NAME_REQUIRED] = temperature_delta[DELTA_TEMPERATURE_NAME] / temperature_delta[
        DELTA_TEMPERATURE_NAME
    ].groupby(temperature_delta.index.year).transform("sum")
