...
from heatpro.demand_profile.hot_water_profile import (
    COLD_WATER_TEMPERATURE_NAME,
    basic_hot_water_monthly_profile,
    basic_hot_water_hourly_profile,
)
...
def sample_cold_water_temperature():
    return pd.DataFrame(
        {COLD_WATER_TEMPERATURE_NAME: np.random.uniform(5, 15, 365)},
...
def sample_monthly_HW_weight():
    return pd.DataFrame(
        {WEIGHT_NAME_REQUIRED: np.random.uniform(0.1, 1, 12)},
...
def sample_raw_hourly_hotwater_profile():
    return pd.Series(
        {WEIGHT_NAME_REQUIRED: np.random.uniform(0.1, 1, 24 * 365)},
...
def test_basic_hot_water_monthly_profile(sample_cold_water_temperature, sample_monthly_HW_weight):
    # Valid case
    monthly_hot_water_profile = basic_hot_water_monthly_profile(
        sample_cold_water_temperature, 70, sample_monthly_HW_weight
    )
    ...
    with pytest.raises(
        ValueError, match="cold_water_temperature and monthly_HW_weight index do not match"
    ):
        basic_hot_water_monthly_profile(
            sample_cold_water_temperature, 70, invalid_monthly_HW_weight
        )
...
def test_basic_hot_water_hourly_profile(sample_raw_hourly_hotwater_profile):
    # Valid case
    hourly_hotwater_profile = basic_hot_water_hourly_profile(
        sample_raw_hourly_hotwater_profile, 0.8, 0.9
    )
    assert WEIGHT_NAME_REQUIRED == hourly_hotwater_profile.name
