...
def setup_data() -> tuple[dict, ExternalFactors]:
    ...
    start_heating_season = pd.to_datetime(
        f"{parameters['Seasons']['SC_start']}-{year}", dayfirst=True
    )
    ...
    return pd.concat(
        (
            ...
            kasuda_soil_temperature(
                ...
                alpha=parameters["Temp_ground"]["cond_ground"]
                * 24
                * 3600
                / (
                    parameters["Temp_ground"]["cp_ground"]
                    * parameters["Temp_ground"]["dens_ground"]
                ),
            ),
        ),
        axis=1,
...
def test_induced_factors_non_regression(induced_factors: pd.DataFrame):
    reference_induced_factors = pd.read_csv(
        "./tests/non_regression/data/induced_factors.csv", index_col=0, parse_dates=True
    )
    for col in reference_induced_factors.columns:
        absolute_gap = (
            induced_factors[col].astype(float) - reference_induced_factors[col].astype(float)
        ).abs()
        assert (
            absolute_gap <= EPSILON * reference_induced_factors[col].astype(float).abs()
        ).all(), f"The relative gap of column '{col}' is over {EPSILON}"


@pytest.fixture
def monthly_building_load(setup_data) -> pd.Series:
    parameters = setup_data[0]
    return pd.Series(
        parameters["C_Neq_housing"]
        * np.array(
            [
                2068,
                1696,
...
@pytest.fixture
def hourly_hot_water_load(monthly_building_load, setup_data) -> HourlyHeatDemand:
    parameters, external_factors = setup_data
    ...
    return special_hot_water(
        ...
        monthly_hot_water_profile=pd.DataFrame(  # How hot water consumption vary each month
            ...
            columns=["weight"],
            index=monthly_building_load.index,
        )
        / 12,
...
@pytest.fixture
def hourly_residential_load(
    monthly_building_load: pd.Series, hourly_hot_water_load: HourlyHeatDemand, setup_data
):
    parameters, external_factors = setup_data
    ...
    monthly_residential_load = (
        monthly_building_load - hourly_hot_water_load.data[ENERGY_FEATURE_NAME].resample("MS").sum()
    ).rename(ENERGY_FEATURE_NAME)

    hourly_residential_profile = basic_building_heating_profile(
        felt_temperature=pd.DataFrame(
            external_factors.temperature.ewm(parameters["Part_SH"]["Text_ponderation"])
            .mean()
            .rename(BUILDING_FELT_TEMPERATURE_NAME)
        ),
...
def hourly_industry_load(monthly_building_load, setup_data):
    parameters, external_factors = setup_data
    yearly_industry_load = (
        monthly_building_load.resample("YS").sum() * parameters["Part_Indu"]["fixed_perc"]
    ).rename("industry")
    ...
    return weekly_weighted_disaggregate(
        ...
        weights=pd.Series(weights, index=external_factors.temperature.index, name="weight"),
    )
...
def hourly_heat_loss_load(monthly_building_load, induced_factors, setup_data):
    parameters = setup_data[0]
    yearly_heat_loss_load = (
        monthly_building_load.resample("YE").sum() * parameters["Heat_Loss"]["fixed_perc"]
    ).rename("heat_loss")
...
def test_demands(district_heating):
    district_heating_reference = pd.read_csv(
        "./tests/non_regression/data/district_heating.csv", index_col=0, parse_dates=True
    )
    for sector, hourly_load in district_heating.demands.items():
        absolute_gap = (
            hourly_load[ENERGY_FEATURE_NAME]
            - district_heating_reference[f"{sector}_{ENERGY_FEATURE_NAME}"]
        ).abs()
        assert (
            absolute_gap
            <= EPSILON * district_heating_reference[f"{sector}_{ENERGY_FEATURE_NAME}"].abs()
        ).all(), f"The relative gap of '{sector}' hourly demand is over {EPSILON}"
...
def test_return_temperature_after_fitting(district_heating):
    ...
    absolute_gap = (
        district_heating.data[RETURN_TEMPERATURE_NAME]
        - district_heating_reference[RETURN_TEMPERATURE_NAME]
    ).abs()
    assert (
        absolute_gap <= EPSILON * district_heating_reference[RETURN_TEMPERATURE_NAME].abs()
    ).all(), f"The relative gap of return temperature after fitting is over {EPSILON}"
